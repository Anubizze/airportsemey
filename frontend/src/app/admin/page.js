...
export default function AdminPage() {
  ...
  const [photoServiceId, setPhotoServiceId] = useState(AIRPORT_SERVICES[0]?.id ?? 'cafe-aspan');
  ...
  const [docSortOrder, setDocSortOrder] = useState(0);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/auth/me`, {
      ...
      .then((response) => {
        ...
        setToken('');
        setMessage('Сессия истекла. Войдите в админку заново.');
      })
      .catch(() => {
        localStorage.removeItem('admin_token');
        setToken('');
  ...
  const loadPartnerDocuments = async (authToken) => {
    try {
      const data = await fetchAdminPartnerDocuments(authToken);
      setPartnerDocuments(Array.isArray(data) ? data : []);
    } catch (error) {
  ...
  const documentGroups = useMemo(
    () => groupDocumentsByYear(partnerDocuments),
    [partnerDocuments],
  );
  ...
  useEffect(() => {
    ...
    void loadPartnerDocuments(token);
  }, [isLogged, token]);
  ...
  const handleDocumentUpload = async (event) => {
    event.preventDefault();
    if (!docFile) {
      setMessage('Выберите файл документа');
      return;
    }
    if (!docTitle.trim()) {
      setMessage('Укажите название документа');
      return;
    }
    setDocUploadLoading(true);
    setMessage('');
    try {
      await uploadPartnerDocument(token, {
        year: docYear,
        file: docFile,
        titleRu: docTitle.trim(),
        titleKz: docTitle.trim(),
        titleEn: docTitle.trim(),
        sortOrder: docSortOrder,
        isPublished: docPublished,
      });
      ...
      await loadPartnerDocuments(token);
    } catch (error) {
      if (!handleAuthError(error)) {
        setMessage(error.message || 'Ошибка загрузки документа');
      }
    } finally {
  ...
  const handleDeleteDocument = async (id) => {
    ...
    setMessage('');
    try {
      await deletePartnerDocument(token, id);
      setMessage('Документ удалён');
      await loadPartnerDocuments(token);
    } catch (error) {
      if (!handleAuthError(error)) {
        setMessage(error.message || 'Ошибка удаления документа');
      }
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
          ...
          <form onSubmit={handleDocumentUpload} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <label className="text-sm">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2"
                value={docYear}
                onChange={(e) => setDocYear(Number(e.target.value) || new Date().getFullYear())}
              />
            </label>
            <label className="text-sm">
              <span className="text-gray-600">Порядок внутри года</span>
              <input
                type="number"
                min="0"
                className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2"
            ...
            <label className="text-sm md:col-span-2">
              <span className="text-gray-600">Название документа</span>
              <input
                required
                className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2"
                value={docTitle}
                onChange={(e) => setDocTitle(e.target.value)}
                placeholder="Годовой отчёт 2024"
              />
            </label>
            <label className="text-sm md:col-span-2">
              <span className="text-gray-600">Файл (PDF или Word, до 20 МБ)</span>
              <input
                type="file"
            ...
            <label className="text-sm flex items-center gap-2 md:col-span-2 mt-6">
              ...
              />
              <span className="text-gray-700">Опубликован на сайте</span>
            </label>
            <div className="md:col-span-4 flex items-center gap-3">
              <button
                type="submit"
                disabled={docUploadLoading || !docFile}
                className="rounded-xl bg-blue-900 text-white text-sm font-semibold px-5 py-2.5 hover:opacity-90 disabled:opacity-50"
              >
                {docUploadLoading ? 'Загрузка...' : 'Добавить документ'}
              </button>
              <button
                type="button"
                onClick={() => void loadPartnerDocuments(token)}
                className="rounded-xl border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-100"
              >
                Обновить
              </button>
          ...
          {documentGroups.length === 0 ? (
            ...
            <div className="space-y-6">
              {documentGroups.map(({ year, documents }) => (
                <div key={year}>
                  <h3 className="text-sm font-semibold text-gray-700 mb-2">{year} год</h3>
                  <div className="overflow-x-auto rounded-xl border border-gray-100">
                    <table className="w-full min-w-[720px] text-sm">
                      <thead>
                        <tr className="border-b border-gray-100 text-left text-gray-500 bg-gray-50">
                          <th className="py-2 px-3">Название</th>
                          <th className="py-2 px-3">Порядок</th>
                      ...
                      <tbody>
                        {documents.map((doc) => (
                          <tr key={doc.id} className="border-b border-gray-50 last:border-0">
                            <td className="py-2 px-3 font-medium text-gray-900">{doc.titleRu}</td>
                            <td className="py-2 px-3 text-gray-600">{doc.sortOrder}</td>
                            <td className="py-2 px-3">
                              <span className={doc.isPublished ? 'text-green-700' : 'text-gray-400'}>
                                {doc.isPublished ? 'Опубликован' : 'Скрыт'}
                              </span>
                            </td>
                            <td className="py-2 px-3">
                              <a
