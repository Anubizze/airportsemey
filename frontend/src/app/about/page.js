...
const TEAM = [
  { name: 'Саматов Ғазизбек Қайратұлы', roleIndex: 0 },
];
...
export default function AboutPage() {
  ...
  const STATS = [
    ...
    { value: '8 500', label: p.terminalArea },
                ...
                {HISTORY.map((event) => (
                  <div key={event.year} className="flex gap-4">
                    ...
                    <div className="flex-1 pt-3.5">
                      <p className="text-gray-700 text-sm">{pickLocalized(event, lang)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {TEAM.map((member) => (
                <div key={member.name} className="bg-white rounded-2xl border border-gray-100 p-5 flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">{p.teamRoles[member.roleIndex]}</div>
                  </div>
                </div>
              ))}
            </div>
