...
const TRANSPORT = {
  ru: [
    ...
    { key: 'bus', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4', items: ['Маршрут №33 — до центра города','Остановка у главного входа в терминал','Интервал: 30 минут','Время в пути: ~40 минут'] },
  ...
  kz: [
    ...
    { key: 'bus', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4', items: ['№33 маршрут — қала орталығына дейін','Терминалдың басты кіреберісінде аялдама','Интервал: 30 минут','Жол уақыты: ~40 минут'] },
  ...
  en: [
    ...
    { key: 'bus', icon: 'M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4', items: ['Route No. 33 — to city centre','Stop at main terminal entrance','Frequency: every 30 minutes','Travel time: ~40 minutes'] },
...
export default function HowToGetPage() {
  ...
  const transports = pickLocalized(TRANSPORT, lang);
  const titles = [p.taxi, p.bus, p.car];
  const [activeMap, setActiveMap] = useState('2gis');
  const twoGisUrl =
  ...
  const route33TwoGisUrl = 'https://2gis.kz/semey/route/23785241346095770';
  const route33YandexUrl =
    'https://yandex.kz/maps/165/semey/routes/bus_33/796d617073626d313a2f2f7472616e7369742f6c696e653f69643d31373034383731383334266c6c3d38302e32333430303925324335302e333638353338266e616d653d333326723d3930303026747970653d627573/';
  const infobusUrl = 'https://infobus.kz/';
  const infobusAppUrl = 'https://play.google.com/store/apps/details?id=kz.soft.infobus';
  const yandexUrl = 'https://yandex.kz/maps/?text=Аэропорт+Семей';
  const mapLink = activeMap === '2gis' ? twoGisUrl : yandexUrl;
  const mapLinkLabel = activeMap === '2gis' ? p.open2gis : p.openMap;
      ...
      <div className="bg-gray-50 min-h-screen py-10">
        <div className="container mx-auto px-4 lg:px-8 max-w-4xl">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden mb-8 shadow-sm">
            <div className="flex gap-2 border-b border-gray-100 bg-white p-3">
              {[
                { key: '2gis', label: '2GIS' },
                { key: 'yandex', label: p.yandexMaps },
              ].map((item) => (
                <button
                  key={item.key}
                  type="button"
                  onClick={() => setActiveMap(item.key)}
            ...
            {activeMap === '2gis' ? (
              <>
                ...
                <div className="border-t border-gray-100 px-4 py-3">
                  <p className="text-sm font-medium text-gray-700">{p.airportName}</p>
                  <p className="text-sm text-gray-500">{p.addressLine}</p>
                  <a
                    href={twoGisUrl}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-2 inline-flex items-center gap-1 text-sm font-medium text-blue-700 hover:text-blue-900 transition-colors"
                  >
                    {p.openMap2gis}
              ...
              <div
                className="flex h-72 items-center justify-center text-gray-400"
                style={{ background: 'linear-gradient(135deg, #e8edf5 0%, #d0dce8 100%)' }}
              >
                <div className="text-center">
                  <svg className="mx-auto mb-3 h-12 w-12 text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-sm font-medium text-gray-500">{p.shortAddress}</p>
                  <a
                    href={mapLink}
                    target="_blank"
                    rel="noopener noreferrer"
          ...
          <section className="mt-8 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">{p.infobusTitle}</h2>
                <p className="mt-1 text-sm text-gray-500">{p.infobusDesc}</p>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-800 transition hover:border-blue-400"
                >
                  {p.app}
                </a>
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <a
                href={route33TwoGisUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-blue-300 hover:text-blue-800"
              >
                {p.route33TwoGis}
              </a>
              <a
                href={route33YandexUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:border-blue-300 hover:text-blue-800"
              >
